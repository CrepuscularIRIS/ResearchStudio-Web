...
# ── parts: gate_legacy — they take this module's namespace explicitly (no import cycle; monkeypatch-safe) ──
import types  # noqa: E402
import gate_legacy  # noqa: E402
...
screen = gate_legacy.screen

_check_schema1_links = gate_legacy._check_schema1_links

def _check_schema2_links(*a, **k):
    return gate_legacy._check_schema2_links(_ns(), *a, **k)


def record_critique(*a, **k):
    return gate_legacy.record_critique(_ns(), *a, **k)


compute_state = gate_legacy.compute_state

def write_packet(*a, **k):
    return gate_legacy.write_packet(_ns(), *a, **k)


def write_verdict(*a, **k):
    return gate_legacy.write_verdict(_ns(), *a, **k)


merge_decision = gate_legacy.merge_decision

def merge(*a, **k):
    return gate_legacy.merge(_ns(), *a, **k)


def budget(*a, **k):
    return gate_legacy.budget(_ns(), *a, **k)


def graveyard(*a, **k):
    return gate_legacy.graveyard(_ns(), *a, **k)


def render_tree(*a, **k):
    return gate_legacy.render_tree(_ns(), *a, **k)


def render_views(*a, **k):
    return gate_legacy.render_views(_ns(), *a, **k)


def render_gapmap(*a, **k):
    return gate_legacy.render_gapmap(_ns(), *a, **k)
