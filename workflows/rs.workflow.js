...
'  ---',
'  MODEL BY CLASS (the model-lock slots) — exhaustive, no ambiguity:',
'  - "opus" (= glm-5.3[1m], GLM fast tier): EXACTLY THREE seat kinds, nothing else —',
'    pattern_summary tagging (ONE call at any paper count), 4.derive, tag-admitted-host-refs.',
'  - "fable" (= claude-opus-5, Opus5): EVERY OTHER llm_subagent emit — 0.4 partition, 0.5',
'    coverage, the generation mega-seat (below), 2.3 coherence, 3.2 audit, 3.3, cite_fix, terms,',
'    recheck, reaudit, 4.fill, fill_fix, 4.1.5, and every failure-path card. When in doubt: fable.',
'',
...
'a context because both are generation-side). The same criterion licenses ONE more merge and',
'forbids all others:',
'- When the emit is "Phase 1 — bottleneck identification" on a FRESH run (no phase1 output yet),',
'  spawn ONE Opus seat covering the WHOLE generation side in one context, three system prompts',
'  chained: (1) bottleneck_identify.txt — write phase1_output.json; (2) IF AND ONLY IF its state',
'  is proceed, run ideate_select.txt — write the 2.1 output; (3) then ideate_generate.txt — write',
'  the 2.2 output. Stage 2/3 re-read NOTHING stage 1 already read: phase1_output.json is in your',
'  context (do not Read it back), lit_table and the corpus are already loaded; open only the',
'  sub-pattern cards the prompts name. If state is do_not_generate, write it and STOP.',
'  Read the two extra prompt files from ' + ENGINE + '/references/system-prompts/ yourself',
'  before spawning (you have Read).',
'- Self-heal is preserved: if the mega-seat returned with phase1 proceed but NO 2.2 output, the',
'  next NAV emits Phase 2 standalone — spawn it as a normal single seat and continue.',
'- NO OTHER GROUPING, EVER. The verifiers stay one-fresh-context-each: 2.3 (the context that',
'  wrote a logic bug rubber-stamps it), 3.2 vs 3.3 (attacker vs answerer), 4.fill vs 4.1.5',
