...
'',
'SEAT DISPATCH — one emit = ONE seat, never split. The workflow agent has NO Agent tool',
'(measured on the 2026-09-08 phyagentos run: the improvised path cost ~10 min of sleep-polling',
'and ~10 min to a 590s timeout kill). The SANCTIONED mechanism is the claude CLI in print mode,',
'FOREGROUND — it returns when the seat is done, so there is NOTHING to poll:',
'',
'  SD=$(mktemp -d /tmp/rs-seats-XXXXXX)',
'  Write the seat prompt (frame below) to $SD/prompt.txt with your Write tool, then ONE Bash:',
'  timeout <T> claude -p --model <M> --strict-mcp-config --mcp-config \'{"mcpServers":{}}\' \',
'    --allowed-tools "<TOOLS>" --max-turns 150 \',
'    < $SD/prompt.txt > $SD/log.txt 2>&1; echo "seat_exit=$?"; tail -c 1500 $SD/log.txt',
'',
'  <T>: 2700 for every seat; 3600 for the generation mega-seat. NEVER 590 — the first mega-seat',
'       attempt was killed at 590s and regenerated whole.',
'  <M>: fable for reasoning seats, opus for the three GLM mechanical seats (table below).',
'  <TOOLS> EXACTLY the class — readwrite "Read Write"; exec "Read Write Edit Bash"; the coverage',
'       seat "Read Write WebSearch". NEVER the full allowlist (the improvised run handed every',
'       seat Bash+Web+Grep — the NO_AUX discipline applies to the CLI path too).',
'  FOREGROUND always: never background the CLI, never seq/sleep file-polling.',
'',
'  The seat prompt written to $SD/prompt.txt is EXACTLY this frame:',
'  ---',
...
'  TYPE TRAP (cost 39 min on the audited run): compute_budget and falsification_prediction are',
'  single PROSE STRINGS — never a JSON object or array in those fields.',
'  ---',
...
'- SEAT-EXIT TYPE GATE (cost 39 min on the audited run): immediately after a seat writes a phase',
'  output, validate it BEFORE the next NAV — python3 ENGINE/scripts/run.py validate --phase2 <file>',
'  (and --phase3/--phase4 as they appear; the entry-point table lists the flags). A failure gets',
'  ONE targeted repair seat NOW (same spawn, prompt = the validator error verbatim + rewrite the',
'  whole file correctly), never left to explode at phase4_skeleton two phases later.',
