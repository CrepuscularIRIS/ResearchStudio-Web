...
const P = A.paths || {}                       // written by dag from .research/harness.json
const TOOLS = P.tools_dir || ".research/tools"
A.search_cmd = A.search_cmd || P.search_cmd || "python3 .claude/skills/paper-search/scripts/search_papers.py"
A.fetch_cmd = A.fetch_cmd || P.fetch_cmd || "python3 " + TOOLS + "/fetch_text.py"
const BUD = Object.assign({ waves: 3, perWave: 4, sketches: 10, keep: 4 }, A.budget || {})
const RSREF = P.rs_ref || ".research/references/idea_spark/references"   // ResearchStudio idea_spark references (external clone; see README)
...
const foundPrompt = (ang) =>
  ...
  "(`python3 " + TOOLS + "/snowball.py <id> --title \"<t>\" --json --max 8`).\n" +
...
const expandAndCollide = await parallel(top.map((s, i) => () => parallel([
  ...
  () => seat(
    ...
    " Also `python3 " + TOOLS + "/search_github.py --query \"<mechanism words>\" --json` " +
