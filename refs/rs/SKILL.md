...
The rc=10 sentinel path still exists as fallback when `--queries` is omitted. The orchestrator: asserts a sane clock; probes connectors and retrieves role-based (arxiv 0-6mo cap 10 / openalex 6-24mo cap 25 / openreview CCF-A 0-18mo cap 25 / **cvf CVPR·ICCV·ECCV full openaccess** 0-18mo cap 25 / elsevier CAS-Q1 0-24mo cap 15 if key set; OpenAlex-priority dedup); extracts URL/ID user-refs from the query into `phase0/user_refs.json`; emits `.pattern_summary_pending` for the host.
