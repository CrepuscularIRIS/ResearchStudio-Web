...
# Multi-source coverage (high-intensity map mode):
#   arxiv + openalex + openreview(CCF-A notes) + cvf(CV full OA) + dblp(CCF-A streams)
#   + elsevier(CAS-Q1 journals, optional key)
CONNECTORS = [
    ('arxiv',           'scripts.search_arxiv',           [],                                       ['feedparser']),
    ('openalex',        'scripts.search_openalex',        [],                                       []),
    ('openreview',      'scripts.search_openreview',      ['OPENREVIEW_USER', 'OPENREVIEW_PASS'],   ['openreview']),
    ('elsevier',        'scripts.search_elsevier',        ['ELSEVIER_API_KEY'],                     []),
    ('cvf',             'scripts.search_cvf',             [],                                       []),
    # DBLP: unified CCF-A conference metadata (NeurIPS/ICML/ACL/KDD/WWW/SIGIR/AAAI/…).
    ('dblp',            'scripts.search_dblp',            [],                                       []),
]

# High-intensity Phase 0 defaults — target ~150–200 unique after dedup (was ~70).
# Falls back gracefully when a connector is unavailable.
PHASE0_CONNECTOR_CONFIG = {
    # arxiv: wider net + larger keep set (still one API call per query)
    'arxiv':           {'window_min': 0, 'window_max': 12, 'max_results': 30, 'max_per_query': 300, 'extra_args': [], 'timeout': 300},
    # openalex: broader published window + semantic booster (enabled in phase0 code)
    'openalex':        {'window_min': 0, 'window_max': 36, 'max_results': 50, 'max_per_query': 60,
                        'extra_args': ['--published-only'], 'timeout': 600},
    # openreview CCF-A notes (partial for CVPR; full CV lives on cvf)
    'openreview':      {'window_min': 0, 'window_max': 24, 'max_results': 40, 'max_per_query': 100,
                        'extra_args': ['--per-venue-cap', '2000'], 'timeout': 1500},
    # elsevier CAS-Q1 journals
    'elsevier':        {'window_min': 0, 'window_max': 36, 'max_results': 30, 'max_per_query': 30,
                        'extra_args': [], 'timeout': 400},
    # cvf full CVPR/ICCV/ECCV openaccess
    'cvf':             {'window_min': 0, 'window_max': 24, 'max_results': 40, 'max_per_query': 60,
                        'extra_args': [], 'timeout': 700},
    # dblp CCF-A multi-stream search (metadata; no abstracts). Pause between streams.
    'dblp':            {'window_min': 0, 'window_max': 36, 'max_results': 50, 'max_per_query': 60,
                        'extra_args': ['--per-stream', '12', '--stream-pause', '1.0'], 'timeout': 180},  # V9: was 1200 — dblp timed out every time from this network (2026-09-06: +20 min phase0, +40 min per collision)
}
...
# openalex (broad peer-reviewed + abstracts) > elsevier (CAS-Q1 venue lock, DOI/cites) >
# openreview (in-review) > arxiv (preprint).
# Prefer sources that carry abstracts / stable venue labels first.
DEDUP_PRIORITY = {
    'openalex': 6,
    'cvf': 5,
    'openreview': 4,
    'elsevier': 3,
    'dblp': 2,   # strong venue coverage, no abstract
    'arxiv': 1,
}
...
def check_connector(label: str, module_path: str, env_keys: list[str],
    ...
    # V9: IDEASPARK_SKIP_SOURCES="dblp,elsevier" drops connectors for this invocation — the Brain
    # workflow sets it after a connector timed out once in Phase 0 (session-wide circuit breaker).
    if label in {x.strip() for x in os.environ.get('IDEASPARK_SKIP_SOURCES', '').split(',') if x.strip()}:
        return False, 'skipped via IDEASPARK_SKIP_SOURCES'
    # 1) packages
    for pkg in packages:
        try:
...
def cmd_phase0(args) -> int:
    ...
    # if openalex failed but at least one of arxiv/openreview worked,
    ...
    published_succeeded = any(s in successes for s in ('openalex',))
    if not published_succeeded:
        print(f'\nNOTE: no published-source connector succeeded (openalex unavailable). '
...
def cmd_phase3_collision(args) -> int:
    ...
    # Probe connectors (collision uses arxiv + openalex + openreview)
    available = []
    for label, module_path, env_keys, pkgs in CONNECTORS:
        ok, reason = check_connector(label, module_path, env_keys, pkgs)
        if ok:
