...
def _api_get(params: dict, timeout: int = 30) -> dict:
    ...
    # rate-limit tier (10 req/s vs 1 req/s anonymous). Prefer OPENALEX_MAILTO,
    # then OPENALEX_EMAIL (PaperSeek / cli naming), then a generic UA.
    mailto = (
        os.environ.get('OPENALEX_MAILTO', '').strip()
        or os.environ.get('OPENALEX_EMAIL', '').strip()
    )
    ua = f'mailto:{mailto} idea-spark/1.0' if mailto else 'idea-spark/1.0'
