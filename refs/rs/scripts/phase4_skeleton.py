...
def _extract_gpu_days(s: str) -> float | None:
    ...
    Also recognizes GPU-hours / A100-hours / H100-hours (converted /24 to
    GPU-days) so envelopes stated in hours (e.g. '<=200 GPU-hours') parse
    correctly instead of falling through to the bare-number fallback."""
    if not s:
        return None
    # Prefer an explicit -day unit first (most specific).
    ...
    if m:
        ...
        return n
    # Then an explicit -hour unit (convert to days). Accept 'hour(s)', 'hr(s)',
    # and the abbreviations 'h' / 'GPU-h' / 'A100-hr' common in budget strings.
    # Allow a few intervening words (e.g. '<=200 total GPU-hours') since prose
    # envelopes often separate the number from the unit. No trailing \b: it
    # breaks on plurals like 'GPU-hours'.
    pattern_h = re.compile(
        r'(\d+(?:\.\d+)?)\s*(?:total\s+|budget\s+|of\s+|up to\s+)?'
        r'(A100|H100|GPU)[-\s]?(?:hours?|hrs?|h)(?![A-Za-z])',
        re.IGNORECASE)
    m = pattern_h.search(s)
    if m:
        n = float(m.group(1)) / 24.0
        unit = m.group(2).upper()
        if unit == 'H100':
            n *= 2.0
        return n
    ...
    # line; the API extractor owns those), never a leading multiplier like
    # '2x RTX' / '8x H100' (a count of devices, not a GPU-day budget), and never
    # a GPU model number (RTX 4090, A100, H100, etc. — these are hardware names,
    # not budgets). If the string only contains hardware/device numbers and no
    # explicit compute-day/hour unit matched above, return None so the caller
    # falls back to the default envelope rather than emitting a phantom budget.
    _HW_MODEL = re.compile(r'(RTX|A100|H100|A10G|L4|T4|V100|MI250|MI300|H200|B200)\s*$',
                           re.IGNORECASE)
    for m2 in _NUM.finditer(s):
        prefix = s[max(0, m2.start() - 5):m2.start()]
        suffix = s[m2.end():m2.end() + 2]
        if '$' in prefix or prefix.upper().endswith('USD') or prefix.upper().endswith('USD '):
            continue
        if suffix.startswith('x') or suffix.startswith('X'):  # '2x', '8X' = device count
            continue
        if suffix.upper().startswith('GB'):  # '48GB', '80GB' = memory size
            continue
        if _HW_MODEL.search(prefix):  # 'RTX 4090', 'A100 80GB' = model number
            continue
