 ...
 *
 * VERBATIM from the Claude Code v2.1.221 native binary, with TWO deliberate edits:
 *   - meta.name changed to 'code-review-extracted' so it does NOT collide with the
 *     built-in 'code-review'.
 *   - the constants the build inlines via ${JSON.stringify(...)} from scattered
 *     minified vars — meta description/whenToUse/phases (Qcp/Zcp/eup), the 5
 *     CORRECTNESS_ANGLES ($cp→_zy), CLEANUP_TEXT (tup = Reuse/Simplification/
 *     Efficiency/Altitude/Conventions joined with blank lines), VERDICT_LADDER
 *     (eZs), VERDICT_LADDER_RECALL (tZs), CLEANUP_PRECEDENCE (kQe),
 *     SWEEP_GAP_FOCUS (j9o), and the inline ${tup.length} (= 5) — are resolved
 *     and inlined here as their exact values, also extracted verbatim from the
 *     same binary.
