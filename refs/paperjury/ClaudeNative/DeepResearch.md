/**
 * deep-research-extracted — RUNNABLE local copy of Claude Code's built-in
 * "deep-research" workflow, VERBATIM from the v2.1.221 native binary with ONE edit:
 * meta.name changed to 'deep-research-extracted' so it does NOT collide with the
 * built-in 'deep-research' (meta description/whenToUse/phases are the binary's own
 * values, resolved from their minified vars). All orchestration / prompts are
 * byte-for-byte original.
 * Run: Workflow({ name: "deep-research-extracted", args: "<your research question>" })
 * Reverse-engineered for reference; original code (c) Anthropic.
 */

export const meta = {
  name: 'deep-research-extracted',
