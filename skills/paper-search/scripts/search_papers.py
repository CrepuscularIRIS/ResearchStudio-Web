...
_SOURCE_MODULE = {
    ...
    # "semantic_scholar": "search_papers_by_semantic_scholar",  # disabled in this deployment
    "crossref": "search_papers_by_crossref",
}
# Default search set: no Semantic Scholar (unreliable anonymous tier / no useful key).
ALL_SOURCES = list(_SOURCE_MODULE.keys())
DEFAULT_SOURCES = ["open_alex", "arxiv", "openreview", "crossref", "dblp"]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Print the ranked, deduped list as JSON and exit (for fetch_text.py)",
    )
    parser.add_argument(
    ...
    if args.raw:
        ...
        if args.json:
            ...
            print(_json.dumps(ranked, ensure_ascii=False, default=str))
            sys.exit(0)
