...
<!-- ARFT/ASI-BENCH BLOCK: kill-attack-discipline -->
<!-- source: ARFT (arXiv 2608.14905) §4 do-not-label, §1 rule 1 polarity, §5 code list · ASI-Bench (arXiv 2608.17271) analysis/reviewer.py:212-284 CAUSE_BUCKET -->
### Step 2.9: Attack Discipline — What Counts as a Kill

The attack memo is worth exactly as much as its weakest point. A memo padded with objections that
cannot survive adjudication trains the next round to discount all of them, so the discipline
below applies before the memo goes to Thread 2. It constrains the attack, never the defence.

#### A. Every attack point names a code, or says none fits

Each point in the memo carries one ARFT code — the closed 45-code vocabulary over ideation (A),
retrieval (B), execution (C), analysis (D), write-up (E), self-review (F) and cross-stage (X) —
plus a locatable citation: a section, a table, a line of the paper. A point that names no code
must refute the two or three nearest ones explicitly. **An attack with no citable location is not
adjudicable and is dropped before Thread 2 ever sees it**, not argued down afterwards.

The codes an attack most often lands on, and the line between the neighbours:

- `A.2` the falsification threshold cannot be crossed even in the best case · `A.5` the metric
  does not reflect the objective · `A.6` the design cannot adjudicate the hypothesis at all.
- `C.1` the result follows deterministically from what the authors wrote · `C.2` an evaluator or
  held-out set was used as a tuning signal · `X.6` the number is real but the mechanism claimed
  did not produce it.
- `D.4` the claim is not entailed by the outputs · `D.5` the baseline that would make it
  meaningful is absent · `E.2` the write-up conceals the negative results · `E.3` a limitation
  that would change the verdict is omitted · `X.4` honest, well-formed, and empty.

#### B. Five attacks that are not attacks

ARFT's do-not-label list, read as an attack filter. Each is a plausible-sounding objection that
adjudication will reject, and putting one in the memo costs the credibility of the points beside
it:

1. **"It disagrees with the source paper."** A clean run that contradicts its source is good
   science. Not `E.2`, not `D.4`, not `X.4`.
2. **"The score was low."** Scores go to zero for reasons unrelated to the work — an unavailable
   judge, a missing observable, a harness fault, and scores are sometimes *anti-correlated* with
   validity. Attack the mechanism or nothing.
3. **"The infrastructure broke."** If the outage is attributed to the host, only the *response*
   to it is attackable — usually `X.7` (kept going down a dead end) or `C.7` (quit at the first
   friction).
4. **"An earlier draft said X."** A retracted finding is a miss. Check the correction log before
   building a point on it.
5. **"There is no evidence they did Y."** What was never observable cannot be charged. Absence of
   evidence is a miss, not a hit.

And the polarity rule that catches the most common bad attack of all: a paper that *audits* itself
for fabrication, leakage or contamination and reports finding none has produced an audited
dimension, not a confession. Quoting its own integrity section back at it as an admission is a
polarity error and will be adjudicated away.

#### C. Say where the fault would live

Every surviving point names a cause bucket — `Environment` | `Task` | `Agent` — where *Task* means
the experiment as designed could not have answered the question however well it ran. A point in
that bucket is the strongest kind of kill available here, because no amount of additional
execution repairs it; a point in `Environment` is usually not a kill at all. Sorting the memo by
bucket before adjudication is what turns a list of objections into a decision.
<!-- END BLOCK: kill-attack-discipline -->
