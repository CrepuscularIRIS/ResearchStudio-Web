...
<!-- ARFT/ASI-BENCH BLOCK: claim-inflation-codes-and-process-score-match -->
<!-- source: ARFT (arXiv 2608.14905) §3.1/§3.2/§3.7, §4 do-not-label, §5 D/E/X families · ASI-Bench (arXiv 2608.17271) analysis/reviewer.py:212-284 dimension 2 -->
### Step 3.9: Name the Inflation Before Routing

The verdict about to be routed is a claim about what the numbers support. Three checks stand
between the numbers and that claim; each names a specific, published failure mode, and each has a
form that looks like diligence when it is actually inflation.

#### A. Does the process match the score?

Two mismatches, in opposite directions, from ASI-Bench's trajectory review:

- **Correct-looking process, low score.** Before accepting a negative result, ask whether the
  scorer was too strict, a format check too rigid, or a gate misfired. A claim of "the method
  does not work" that rests on a broken comparison is as wrong as an inflated positive one, and
  it is the cheaper mistake to make.
- **Wrong process, high score.** The dangerous direction. The metric moved, but through a bug, a
  leak, or luck rather than the claimed mechanism — ARFT `X.6 Right-for-the-Wrong-Reason`, which
  *requires a real success to explain*. If the process cannot account for the number, the claim
  is not supported no matter how good the number is.

State which of the two, if either, applies, with the evidence. "Neither" is a legitimate answer
and must be written down rather than left implied.

#### B. Code the gap between the outputs and the claim

The claim-time family, with the distinctions that keep them from collapsing into one another:

| Code | What it is | What it is not |
|---|---|---|
| `D.4` Method-Conclusion Disconnect | the claim is not *entailed* by the outputs | not a wrong number — the number can be right and the claim still unsupported |
| `D.1` Artifacts as Insights | numbers are real, but a bug or noise is read as a result | distinct from `D.6`: misinterpretation, not invention |
| `D.6` Result Hallucination | metrics, tables or charts that were never computed | if it was computed and misread, it is `D.1` |
| `D.3` Statistical Misuse | no significance test, no interval, no uncertainty | not "the effect is small" — that is a finding, not a failure |
| `D.5` Baseline & Ablation Deficit | the comparison that would make the claim meaningful is missing | not "more experiments would be nice" |
| `E.2` Overclaiming & Selective Narrative | the write-up exaggerates and conceals negative results | belongs to write-up, not to analysis |
| `E.3` Omission of Critical Limitations | a core invalidating limitation is left out | not every caveat — the ones that would change the verdict |
| `X.4` Honest-but-Hollow | well-formed, honest, and carrying no substance | the hardest to admit, and the one no single number catches |

If the results support a weaker claim than the one proposed, say which weaker claim they support.
Downgrading here is the normal outcome and it is not a failure of the experiment.

#### C. What must not be called a failure

ARFT's do-not-label list, applied to this gate. Each of these is a way of manufacturing a problem
that is not there:

1. **Honest disagreement with a source paper is not a failure.** A clean run that contradicts the
   paper it came from is good science — it is not `E.2`, not `D.4`, not `X.4`.
2. **A low score is not a failure.** Scores go to zero for reasons unrelated to the work: an
   unavailable judge, a missing observable, a harness fault. *Label the mechanism, never the
   score.*
3. **An infrastructure outage the audit attributes to the host is not our failure.** Only the
   response to it is.
4. **A retracted finding is a miss.** Check what an earlier pass withdrew before citing it.
5. **What could not be observed is a miss, not a hit.** Absence of evidence about a stage that was
   never logged is not evidence of a failure in it.

The verdict that leaves this step therefore carries, alongside the seven fields the gate already
requires: the **codes** it is charging (with HIT or PARTIAL and a locatable citation each), the
**process/score mismatch** finding, and — where the claim was reduced — **the weaker claim the
results do support**. A verdict that charges no code states that explicitly; silence is not a
clean bill.
<!-- END BLOCK: claim-inflation-codes-and-process-score-match -->
