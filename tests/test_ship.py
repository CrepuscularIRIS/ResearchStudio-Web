...
from test_stage import ws, write_map, baseline  # noqa: E402
...
def test_ship_refusals_and_stage_d(tmp_path):
    ...
    assert d["stage"] == "D" and "exit (a)" in d["reason"] and "bundle.py write" in d["next"][0]
    (r / "gates" / "write-review.json").write_text(json.dumps({"verdict": "pass"}))
